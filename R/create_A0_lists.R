...
create_A0_list_for_ATE_tg <- function(t, g, g_list, t_list, N_g_list){
  ...
  A0_treated_list <- purrr::map(.x = 1:length(g_list), A0_g_treated_fn)
  A0_control_list <- purrr::map(.x = 1:length(g_list), createControlWeights_helper )
...
create_A0_list_for_ATE_calendar_t <- function(t, g_list, t_list, N_g_list){
  ...
  A0_lists <- purrr::map(.x = treated_by_t_indices,
                              .f = ~ scalar_product_lists(N_g_list[[.x]]/N_total_treated ,
                                                          create_A0_list_for_ATE_tg(t =t, g = g_list[.x], g_list = g_list ,t_list = t_list, N_g_list = N_g_list) ) )
...
create_A0_list_for_ATE_cohort_g <- function(g, g_list, t_list, N_g_list){
  ...
  A0_lists <- purrr::map(.x = treated_period_indices,
                              .f = ~ scalar_product_lists(1/T_treated ,
                                                          create_A0_list_for_ATE_tg(t =t_list[.x], g = g, g_list = g_list ,t_list = t_list, N_g_list = N_g_list) ) )
...
create_A0_list_for_simple_average_ATE <- function(g_list, t_list, N_g_list){
  ...
  A0_lists <- purrr::map(.x = 1:NROW(gt_df),
                              .f = ~ scalar_product_lists(gt_df$N_g[.x]/N_total,
                                                          create_A0_list_for_ATE_tg(t = gt_df$t[.x], g= gt_df$g[.x], g_list = g_list ,t_list = t_list, N_g_list = N_g_list) ) )
